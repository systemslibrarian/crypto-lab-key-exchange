 ...
 *     this file replaces called `revealAll`, which force-opened every
 *     `<details>`, stripped `[hidden]` off every element — including the
 *     keyboard-shortcuts dialog, which then covered the page, a side effect it
 *     had to work around by reloading between scans — and injected
 *     `animation: none !important`. That injection was actively self-defeating
 *     here: `.math-row` sets `opacity: 0` and reaches its visible state only
 *     through `row-enter ... forwards`, so cancelling the animation left every
 *     derivation row invisible to the very scanner that was meant to measure it.
 *
 ...
 *     nothing, and most of this lab's output panels are empty until a run.
 *
...
export async function boot(page: Page, theme: 'dark' | 'light'): Promise<void> {
  ...
  await expect(page.locator('#dh-output')).toBeVisible();
  await expect(page.locator('#mitm-relay')).toBeVisible();
 ...
 * plausible offender: it prints long modular-arithmetic rows, a curve plot and
 * several fixed-minimum grid tracks (`minmax(260px, 1fr)`, `minmax(160px, 1fr)`).
 */
 ...
 *
 * Every exhibit here computes into an output panel that is empty until its
 * button is pressed, so a gate that only scans first paint measures a page of
 * placeholders. Both MitM outcomes are driven, because the relayed attack and
 * the signature-aborted defence render different markup.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  await page.locator('#dh-attack').click();
  await expect(page.locator('#dh-attack-output')).not.toBeEmpty();
  await scan(page, `${theme} / DH discrete-log attack`);

  await page.locator('#ec-walk').click();
  await expect(page.locator('#ec-output')).not.toBeEmpty();
  await scan(page, `${theme} / EC scalar walk`);

  await page.locator('#kem-run').click();
  await expect(page.locator('#kem-output, #hybrid-out-chip').first()).toBeVisible();
  await scan(page, `${theme} / ML-KEM run`);

  await page.locator('#hybrid-run').click();
  await expect(page.locator('#hybrid-out-chip')).toBeVisible();
  await scan(page, `${theme} / hybrid combiner`);

  await page.locator('#mlwe-resample').click();
  await expect(page.locator('#mlwe-output')).not.toBeEmpty();
  await scan(page, `${theme} / MLWE resample`);

  await page.locator('#mlwe-solve').click();
  await expect(page.locator('#mlwe-output')).not.toBeEmpty();
  await scan(page, `${theme} / MLWE solve`);

  await page.locator('#shor-random-a').click();
  await expect(page.locator('#shor-output')).not.toBeEmpty();
  await scan(page, `${theme} / Shor period finding`);

  // The MitM relay: the tampering attack, then the authenticated defence. Two
  // different verdict surfaces off the same panel.
  ...
  await scan(page, `${theme} / MitM relay succeeds`);

  await page.locator('#mitm-authrun').click();
  await expect(page.locator('#mitm-verdict')).toBeVisible();
  await scan(page, `${theme} / MitM defeated by signatures`);
}
